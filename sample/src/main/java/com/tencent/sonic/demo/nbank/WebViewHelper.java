...
public class WebViewHelper {
    ...
    public static String execJsFunction(WebView webView, String function, Object... params) {
        ...
        if (TextUtils.isEmpty(function)) {
            Logs.ee("execJsFunction,function参数为空，无法构造js代码", TAG);
            return null;
        }

        String js = genJsCode(function, params);
        if (!TextUtils.isEmpty(js)) execJsCode(webView, js);
        Logs.dd("execJsFunction,执行js函数，" + js, TAG);
        return js;
    }
    ...
    public static void execJsCode(WebView webView, String js) {
        ...
        if (TextUtils.isEmpty(js)) {
            Logs.ee("execJsCode,要执行的js代码为空，无法执行", TAG);
            return;
        }
        ...
        webView.loadUrl(_js);
        Logs.dd("execJsCode,执行js代码，" + js, TAG);
    }
    ...
    public static String genJsCode(String function, Object... params) {
        //empty检查
        if (TextUtils.isEmpty(function)) {
            Logs.ee("genJsCode,function参数为空，无法构造js代码", TAG);
            return null;
        }
        //函数名
        ...
        if (params != null || params.length != 0) {
            for (int i = 0; i < params.length; i++) {
                //空指针判断
                if (params[i] == null || TextUtils.isEmpty(params[i].toString())) continue;
